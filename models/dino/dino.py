...
from .DA_utils import mask_to_box_single_img
...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        #-----制备da使用的mask掩膜（原图大小）
        mask_flatten_da_class = None
        if self.training:
            if targets is not None:
                image_size = samples.tensors.shape[-2:]
                #处理标注
                box_masks = []
                mask_labels = []
                for target in targets:
                    box_mask,mask_label = mask_to_box_single_img(target["boxes"], target["labels"], target['size'], image_size,num_classes = self.num_classes)
                    box_masks.append(box_mask)  #(num_class,h,w)
                    mask_labels.append(mask_label)  #(num_class,h,w)
                box_masks = torch.stack(box_masks, dim=0) # (b,num_class,h,w)
                mask_labels = torch.stack(mask_labels, dim=0)# (b,num_class)
                #---debug---------------
                #import cv2
                # numpy_img = samples.tensors[0,:,:,:].cpu().numpy()
        ...
        # #----DA Process
        if self.training:
            query_feature = da_output['querys_all_layer'] #(batch_size,layer:6,num_class,256)
            #query_feature = query_feature[:,-1,:,:] #(batch_size,num_class,256)


            da_output['query_DA'] = {'query_features': query_feature,  #聚合的各类别特征 (batch_size,num_class,256)
                                    'class_map': mask_labels,  #用于图像中的存在类别  (batch_size,num_class)
                                     }
...
class SetCriterion(nn.Module):
    ...
    def loss_da(self, outputs):
        ...
        query_features = outputs['query_features'] # #(batch_size,layer:6,num_class,256)
        class_map = outputs['class_map']         #  (batch_size,num_class) 对齐维度
        B,layernum,class_num,_ = query_features.shape

        class_map_source = class_map[:B//2,:]
        class_map_target = class_map[B//2:,:]

        class_map_source = (torch.sum(class_map_source,dim=0) != 0) * 1.0
        class_map_target = (torch.sum(class_map_target,dim=0) != 0) * 1.0

        class_map_final = ((class_map_source +class_map_target ) == 2.0) * 1.0

        loss_all = 0
        for idx in range(layernum):
            source_query_features = query_features[:B//2,idx,:,:].mean(dim=0) # (num_class,256)
            target_query_features = query_features[B//2:,idx,:,:].mean(dim=0)

            # (1)归一化
            source_query_features = F.normalize(source_query_features, dim=1)
            target_query_features = F.normalize(target_query_features, dim=1)

            # (2)计算相似性
            logits = source_query_features.mm(
                target_query_features.permute(1, 0).contiguous())  # (class_num,C) * (C,class_num) = （class_num,class_num）

            # # (3)制备标签
            cons_label = torch.eye(class_num).to(logits.device)
            cons_label = cons_label * class_map_final
            #
            # (4)计算损失
            ce_criterion = nn.CrossEntropyLoss()
            loss = ce_criterion(logits, cons_label)
            loss_all += loss
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if 'da_output' in outputs:
            ...
            # 1.query_DA
            losses['loss_query_DA'] = self.loss_da(da_output['query_DA'])
...
def build_dino(args):
    ...
    # for DA training:
    weight_dict['loss_query_DA'] = args.querys_da_loss_coef
    print(weight_dict['loss_query_DA'])
    print('!!!!!!!!!!!')
