...
from .DA_attention import DomainAttention
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if self.training:
            self.multi_view_query = nn.Parameter(torch.empty(1,self.num_classes ,d_model))  #(batch,class_num ,channel)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None,da_mask = None):
        """
        Input:
        ...
        # ----DA:多视角探针（query）
        multi_view_query = None
        da_output = {}
        if self.training:
            multi_view_query = self.multi_view_query.expand(src_flatten.shape[0],-1, -1)  # (batch,class_num,256)


        #########################################################
        # Begin Encoder
        #########################################################
        memory, enc_intermediate_output, enc_intermediate_refpoints, multi_view_query_all_layer = self.encoder(
                src_flatten, 
                ...
                da_query=multi_view_query,
                da_atten_mask = da_mask
                )
        # ----DA 处理
        if self.training:
            da_output['querys_all_layer'] = torch.stack(multi_view_query_all_layer,
                                                                   dim=1)  # (batch_size,layer:6,class_num,256)
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal,da_output #增添DA返回值
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
            ...
            da_query: Tensor = None,
            da_atten_mask: Tensor = None
            ):
        """
        Input:
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not dropflag:
                if self.deformable_encoder: #默认跑这个,增添DA处理
                    output, da_query = layer(src=output, pos=pos, reference_points=reference_points,
                                               spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                               key_padding_mask=key_padding_mask, input_da_query=da_query,input_da_atten_mask = da_atten_mask)
                    da_query_save_all_layer.append(da_query)
                else:
        ...
        return output, intermediate_output, intermediate_ref,da_query_save_all_layer
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None,input_da_query = None,input_da_atten_mask = None):
        # self attention
        ...
        # -------DA 处理:
        if self.training:
            input_da_query = self.space_attn(input_da_query, src, pos, input_da_atten_mask)  # (batch,class_num,256)

        return src,input_da_query
